...
clear;
clc;
%% Simulink Simulation Time
SimTime = 25;
%% Pressure Feedback Tuning Parameters
% HP Filter
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
kF_pf_Up = 0.7; % select between 0.5 and 6.5
tF_pf_Up = 2; % select between 2 and 5 % The HP time constant (2/wn) < tHPA < (5/wn)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
kq_xS_PB = 4.8946e-04;
wn_pf=17.45;
% CA_down @ 450 mm
...
%% Switch to Exp. result with smaller valve opeing (uS_sp have to be changed accordently) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Up=1;
if Up == 1
x_cyl_0 = 0.05;        % [m]
...
p0_B=1.213;                  % [bar]     Initial pressure rod side chamber
p0_A=62.79;                  % [bar]     Initial pressure piston side chamber
p0_A1=1.08;                    % [bar]     Initial pressure ocv-dcv volume
p0_C=26.35;                    % [bar]     Initial pressure pcomp-dcv volume
end 
...
% Valve settings
DB = 0;         % Valve Deadband
uS_sp = -0.4;       % Valve opening Set Point [-1....1] 
uS0 = -0.00;         % Initial Valve Opening [-1....1] 
%% Volumes
% Parameter given in liter (From Damiano)
V_CH_A = 1.0e-03;
V_CH_A1 = 0.1e-03;
V_CH_B = 1.25e-03; 
...
p_ocv_set=196;               % [bar]     Crack pressure of the relief valve
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mu_ocv=10;                   % [-]       Pilot ratio
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
k_ocv=15;                   % [l/min/sqrt(bar)] Flow coefficient
p_ocv_fo=350;               % [bar]     Pressure to fully open
tau_ocv=0.0889*0+0.001;
%tau_ocv=0.0889*0+0.023;          % [s]       Time constant for valve opening dynamics
% [s]       Time constant for valve opening dynamics
...
tau_pc= tau_ocv;             % [-]       Time constant pc dynamics
%tau_pc= 0.0064*0+0.05;             % [-]       Time constant pc dynamics
...
k_DCV_PA= 7.85;           % [(L/min)/(bar^0.5)] (PUMP TO ACTUATOR A-PORT)
k_DCV_PB= 7.55;           % [(L/min)/(bar^0.5)] (PUMP TO ACTUATOR B-PORT)
k_DCV_T= 7.85;                  % [(L/min)/(bar^0.5)]  (ACTUATOR TO TANK)
%% Oil parameters
K_oil=1.5e4*0+12000;                % [bar]     Oil Bulk modulus
...
muC = (Bore_diameter^2)/(Bore_diameter^2-Rod_diameter^2);
% Cyl areas
...
q2=0.00019622;                 % [rad] Revolute joint position
q3=0.00039275;                  % [rad] Revolute joint position
q4=0.00058958;                 % [rad] Revolute joint position
q5=0.00039335;                 % [rad] Revolute joint position
q6=0.0016877;                   % [rad] Revolute joint position
q7=0.0026297;                   % [rad] Revolute joint position
q8=0.0019103;                   % [rad] Revolute joint position
q9=0.0012173;                   % [rad] Revolute joint position
q10=0.00055053;                 % [rad] Revolute joint position
q11 = -0.19953;
q12 = 2.9434;
q13 =-1.4629;
q14 = 1.6643;
...
% sim('non_linear_MBS_PF_v04',[],options)
